...
if model_name == 'adnc':
    ...
    expt_dir = "experiments/pre_trained/cnn_rc_task/biadnc"
...
with tf.Session(config=conf) as sess:
    ...
    for _ in tqdm(range( int(dl.batch_amount('valid')))):
        ...
        verror = sess.run([error_rate],feed_dict={data: sample['x'], answer_idx:sample['answer_idx'], candidates_mask:sample['candidates']})
        valid_error += verror
        valid_count += 1
    ...
    for _ in tqdm(range( int(dl.batch_amount('test')))):
        ...
        terror = sess.run([error_rate],feed_dict={data: sample['x'], answer_idx:sample['answer_idx'], candidates_mask:sample['candidates']})
        test_error += terror
        test_count += 1
